...
*/
var personas:Array<Persona>= new Array<Persona>; //Array del modulo para que pueda ser usada por todas las funciones del mismo que almacenará las personas definidas
...
function main(){
    ...
    let error:string=modificaPersona("00000000T");//Intenta modificar a la persona con dni X y devuelve el error si no lo consigue
    if (error==""){
...
function definirPersonasIniciales(): void {
    ...
    let dir_person:Array<Direccion>;
    ///Creo una variable array de telefonos para almacenar las direcciones de las personas
    let tel_person:Array<Telefono>;
    //Creo una variable array de mails para almacenar las direcciones de las personas
    let mail_person:Array<Mail>;
    let notas_person:Array<string>;

    //DEFINICIÓN DE PERSONAS
    //1. Primera persona
    //Añado una direccion en el array de direcciones
    dir_person=new Array<Direccion>();
    dir_person.push(new Direccion("Avenida Andreu", "51520", "Os Razo Medio", "Valladolid", 581, "Ático 7","C"));
    //Añado un teléfono a la persona 1
    tel_person= new Array<Telefono>();
    tel_person.push(new Telefono("+34 914285715"));
    //Añado un teléfono a la persona 1
    mail_person=new Array<Mail>();
    mail_person.push(new Mail("persona1@host.es"));
    //Creo la perimera persona con todos los datos menos las notas (que son opcionales)
    let persona1:Persona= new Persona("Rodrigo", "Ávalos Sánchez", "00000000T", new Date("17/07/1991"),
                                dir_person, mail_person, tel_person, "Verde", "Hombre");
    ...
    //Añado una direccion en el array de direcciones
    dir_person= new Array<Direccion>();
    dir_person.push(new Direccion("Travessera Gil", "86935", "Vall Tejada", "Segovia", 60, 9,"F"));
    //Añado un teléfono a la persona 1
    tel_person= new Array<Telefono>();
    tel_person.push(new Telefono("+34 601201236"));
    //Añado un teléfono a la persona 1
    mail_person=new Array<Mail>();
    mail_person.push(new Mail("persona2@host.com"));
    mail_person.push(new Mail("persona2_secundario@host.com", "Secundario"));
    //Añado un par de notas
    notas_person= new Array<string>();
    ...
    let persona2:Persona = new Persona("Sandra", "Gil Romero", "35238923W", new Date("20/04/1996"),
                                dir_person, mail_person, tel_person);
    ...
    //Añado una direccion en el array de direcciones
    dir_person=new Array<Direccion>();
    dir_person.push(new Direccion("Avinguda Castañeda", "12867", "San Pulido de las Torres", "Segovia", "Bloq. 2", 4));
    //Añado un teléfono a la persona 1
    tel_person= new Array<Telefono>();
    tel_person.push(new Telefono("+34 679092726"));
    tel_person.push(new Telefono("+34 920102167", "Fijo"));
    //Añado un teléfono a la persona 1
    mail_person=new Array<Mail>();
    mail_person.push(new Mail("persona3@host.com"));
    //Añado un par de notas
    notas_person= new Array<string>();
    notas_person.push("Tiene dos teléfonos.")
    notas_person.push("El numero de la dirección es una cadena de caracteres.")
    //Creo la perimera persona con todos los datos menos el color favorito y el sexo (opcionales)
    let persona3:Persona = new Persona("Marina", "Calderón Gutierrez", "99999999R", new Date("05/12/2001"),
                                dir_person, mail_person, tel_person, "Azul", "Mujer");

    personas.push(persona3); //añade la persona 2 al array de personas
}

function modificaPersona(dni:string):string{
...
function localizaPersona(dni:string):Persona|undefined{
    ...
    return personas.filter(obj =>{return obj.dni===dni})[0]; 
}


function agregaDatos(personaEncontrada:Persona){
    //Agrega los datos a la persona a modificar
    //Direccion
    personaEncontrada.agregaDireccion(new Direccion("Av de Elvas", "06006", "Badajoz", "Badajoz")); //Agrega direccion
    personaEncontrada.agregaTelefono(new Telefono("Laboral", "+34666666666")); //Agrega Telefono
    personaEncontrada.agregaMail(new Mail("Laboral", "trabajo@hostLaboral.com")); //Agrega Mail
}
